...
zip -qr "build/packages/api-${SHORT_SHA}.zip" gen_ai_on_aws

# Clean up the temporary version file
rm gen_ai_on_aws/version.py

echo "✓ Package created at: ${RELATIVE_PATH}build/packages/api-${SHORT_SHA}.zip"
echo ""
echo "Summary:"
echo "  Layer:   ${RELATIVE_PATH}${LAYER_ZIP}"
echo "  App:     ${RELATIVE_PATH}build/packages/api-${SHORT_SHA}.zip"
echo ""
